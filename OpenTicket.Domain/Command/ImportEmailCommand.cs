...
namespace OpenTicket.Domain.Command
{
    ...
    {
        public class Handler : IRequestHandler<ImportEmailCommand>
        {
            ...
            public Handler(IMediator mediator, IMapper mapper,
                IEnumerable<IMailClientFactory> mailClientFactories)
            {
                _mapper = mapper ?? throw new ArgumentNullException(nameof(mapper));
                var clientFactories = mailClientFactories as IMailClientFactory[] ??
                    mailClientFactories.ToArray();
                if (mailClientFactories == null || !clientFactories.Any())
                    throw new ArgumentNullException(nameof(mailClientFactories));
            ...
            {
                var emailAccounts =
                    (await _mediator.Send(new QueryEmailAccounts { IsActive = true }, cancellationToken))
                    .ToArray();
                foreach (var emailAccount in emailAccounts)
                {
                    var mailClientFactory = _mailClientFactories.FirstOrDefault(factory =>
                        factory.SupportedProtocol == emailAccount.Protocol);
